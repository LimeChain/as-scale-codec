...
export function demonstrate(): void {
    ...
    trace("String [0x04, 0x61] -> " + decodeStr.value.toString());
    ...
    // TODO -> Workaround related to a GC issue
    __retain(changetype<usize>(scaleString));
    __retain(changetype<usize>(scaleHash));
    __retain(changetype<usize>(decodeStr));
    __retain(changetype<usize>(decodeHash));
}