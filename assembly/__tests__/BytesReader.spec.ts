...
describe("BytesReader", () => {
    it("decodes single type", () => {
        ...
        assert(buffer.readInto<Bool>() == new Bool(false), "Not correctly decoded");
    })
    ...
    it("decodes many types", () => {
        ...
        expect<Hash>(buffer.readInto<Hash>()).toStrictEqual(new Hash(hashBytes));
        expect<Bool>(buffer.readInto<Bool>()).toStrictEqual(new Bool(true));
        expect<ScaleString>(buffer.readInto<ScaleString>()).toStrictEqual(new ScaleString("nice"));
    })
    ...
    it("decodes list of signed integers", () => {
        ...
        expect<Int32>(buffer.readInto<Int32>()).toStrictEqual(new Int32(-570425345));
    ...
    it("decodes list of CompactInts correctly", () => {
        ...
        const buffer = new BytesReader(bytes);
        expect<CompactInt>(buffer.readInto<CompactInt>()).toStrictEqual(new CompactInt(1));
        expect<CompactInt>(buffer.readInto<CompactInt>()).toStrictEqual(new CompactInt(10));
        expect<CompactInt>(buffer.readInto<CompactInt>()).toStrictEqual(new CompactInt(145));
        expect<CompactInt>(buffer.readInto<CompactInt>()).toStrictEqual(new CompactInt(69));
        expect<CompactInt>(buffer.readInto<CompactInt>()).toStrictEqual(new CompactInt(12312411));
    })

    it("decodes byteArray correctly", () => {
        const bytes: u8[] = [40, 12, 123, 1, 21, 12, 33, 12, 21, 12, 1];
        const buffer = new BytesReader(bytes);
        expect<ByteArray>(buffer.readInto<ByteArray>()).toStrictEqual(new ByteArray(bytes.slice(1)));
    })

    it("decodes ScaleString correctly", () => {
        const bytes: u8[] = [8, 97, 115, 20, 115, 99, 97, 108, 101, 20, 99, 111, 100, 101, 99];
        const buffer = new BytesReader(bytes);
        expect<ScaleString>(buffer.readInto<ScaleString>()).toStrictEqual(new ScaleString("as"));
        expect<ScaleString>(buffer.readInto<ScaleString>()).toStrictEqual(new ScaleString("scale"));
        expect<ScaleString>(buffer.readInto<ScaleString>()).toStrictEqual(new ScaleString("codec"));
    })
